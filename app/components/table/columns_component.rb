class Table::ColumnsComponent < Table::TableComponent
  def initialize(columns, class_name:, show_actions:)
    @columns = columns
  ...
  def table_heads
    @columns.map do |column|
      content_tag :th, class: "px-6 py-3 text-left", scope: "col" do
        t("activerecord.attributes.#{@class_name.to_s.downcase}.#{column}")
      end
    end.join.html_safe
  end
